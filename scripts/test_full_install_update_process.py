...
Non-interactive paths use env flags documented in ``scripts/install.sh`` and
``scripts/update.sh``. Optional ``--with-interactive`` appends TTY-driven scenarios
...
Usage (from repository root):
  ...
  python scripts/test_full_install_update_process.py --profile release --with-interactive --skip-http
"""
...
class Harness:
    ...
    def strip_env_local_profile_sync_keys(self, container: str) -> None:
        """Remove ``CH_UPDATE_SYNC_*`` lines so ``update.sh`` can prompt on a TTY."""
        self.docker_exec(
            container,
            r"""python3 - <<'PY'
from pathlib import Path
...
""",
    ...
    def manifest_data_dir(self, container: str, root: str) -> str:
        ...
        ``agents.discovery.json`` is regenerated by ``update.sh`` / ``discover-agents.mjs``
        ...
        return self.docker_exec_capture(
            ...
            f"! -path './agents.discovery.json' | LC_ALL=C sort | xargs -r sha256sum\n",
            workdir="/",
        )
...
log_user 1
set timeout -1
set cmd {cd /workspace && unset CI CH_INSTALL_NONINTERACTIVE && exec bash scripts/setup.sh}
spawn bash -lc $cmd
...
""",
    ...
    def scenario_update_interactive_sync_decline(self) -> None:
        """TTY ``update.sh`` with unset sync flag: decline bundled profile sync."""
        ws = self.temp_workspace()
        ...
        try:
            ...
            exp = r"""
# update.sh (TTY, unset CH_UPDATE_SYNC_*): Sync bundled profile templates now? [y/N]:
log_user 1
set timeout -1
set cmd {cd /workspace && unset CI CH_INSTALL_NONINTERACTIVE CH_UPDATE_SYNC_HERMES_PROFILE_TEMPLATES && exec bash scripts/update.sh}
spawn bash -lc $cmd
...
"""
    ...
    def scenario_update_interactive_sync_accept(self) -> None:
        """TTY ``update.sh`` with unset sync flag: accept bundled profile sync."""
        ws = self.temp_workspace()
...
log_user 1
set timeout -1
set cmd {cd /workspace && unset CI CH_INSTALL_NONINTERACTIVE && export REPO_URL=file:///tmp/ch-hub-bare.git BRANCH=dev INSTALL_DIR=/tmp/ch-install-harness && rm -rf /tmp/ch-install-harness && exec bash scripts/install.sh}
spawn bash -lc $cmd
